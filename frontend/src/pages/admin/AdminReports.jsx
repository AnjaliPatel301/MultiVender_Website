...
export default function AdminReports() {
  ...
  return (
    <AdminPageWrapper title="Reports & Analytics">
      {/* Controls */}
            ...
            <div className="grid grid-cols-1 xs:grid-cols-2 gap-2">
              {REPORT_TYPES.map(rt => (
                <button key={rt.value} onClick={() => setReportType(rt.value)}
                  ...
                  <span>{rt.icon}</span> <span className="truncate">{rt.label}</span>
                </button>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col xs:flex-row gap-2 xs:items-center">
                ...
                <span className="text-gray-400 text-xs text-center xs:text-left">to</span>
                <div className="relative flex-1 min-w-0">
                  <FiCalendar className="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
      ...
      {summary && Object.keys(summary).length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
          {Object.entries(summary).map(([key, val]) => (
            <div key={key} className="bg-white rounded-xl p-3 sm:p-4 shadow-sm border border-gray-100 min-w-0">
              <p className="text-xs text-gray-500 capitalize mb-1 truncate">{key.replace(/([A-Z])/g, ' $1')}</p>
      ...
      {data && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          {data.length === 0 ? (
            <div className="text-center py-12 text-gray-400 px-4">No data for selected period</div>
          ) : (
                ...
                <thead className="bg-gray-50">
                  <tr>
                    {Object.keys(data[0]).map(h => (
                      <th key={h} className="text-left px-3 sm:px-4 py-3 text-xs font-medium text-gray-500 uppercase whitespace-nowrap">
                        {h.replace(/([A-Z])/g, ' $1')}
                      </th>
                    ))}
                  </tr>
                  ...
                  {data.map((row, i) => (
                    <tr key={i} className="hover:bg-gray-50">
                      {Object.values(row).map((val, j) => (
                        <td key={j} className="px-3 sm:px-4 py-3 text-sm text-gray-700 whitespace-nowrap">{String(val)}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
