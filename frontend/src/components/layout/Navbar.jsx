...
export default function Navbar() {
        ...
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => isAuthenticated ? navigate('/wishlist') : navigate('/login')}
              className="relative p-2.5 text-gray-600 hover:text-red-500 transition-colors hidden md:flex items-center">
              <FiHeart className="w-6 h-6" />
              {wishlistCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-xs min-w-4 h-4 rounded-full flex items-center justify-center px-1 font-bold">{wishlistCount}</span>
              )}
            </button>
            ...
            {isAuthenticated ? (
              <div className="relative">
                <button onClick={() => setUserMenuOpen(!userMenuOpen)}
                  ...
                  <div className="w-8 h-8 bg-gradient-to-r from-red-500 to-red-500 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
  ...
  {activeMega === cat.slug && cat.types?.length > 0 && (
    <motion.div
      ...
      onMouseLeave={closeMega}
      className="absolute top-full left-0 mt-0 bg-white border border-gray-200 shadow-2xl rounded z-50 overflow-hidden"
      style={{ minWidth: '400px', maxWidth: '700px' }}
    >
      {/* Header */}
      <div className="bg-gradient-to-r from-red-600 to-red-500 px-3 py-2 flex items-center justify-between">
        <button
          onClick={() => navToShop(cat.slug)}
          className="text-white font-bold text-base hover:underline flex items-center gap-1"
        >
          {cat.name}
      ...
      {/* Simple columns — grouped or flat */}
      <div
        className="p-6 grid gap-x-8 gap-y-1"
        style={{
          gridTemplateColumns: `repeat(${Math.min(Math.ceil(cat.types.length / 6), 3)}, minmax(140px, 1fr))`,
          maxHeight: '400px',
          overflowY: 'auto',
        }}
      >
        {cat.types.map((subType, i) => (
          <button
            key={subType}
