    ...
    >
      <Link to={`/product/${product._id}`}>
        ...
        <div className="relative overflow-hidden bg-gray-50" style={{ aspectRatio: '3/4' }}>
          ...
          <img
            src={product.images?.[0] || 'https://via.placeholder.com/300x400?text=TECAISHOP'}
            alt={product.name}
          ...
          <div className="absolute top-3 right-3 flex flex-col gap-2">
            <motion.button
              ...
              whileTap={{ scale: 0.9 }}
              className={`w-9 h-9 rounded-full flex items-center justify-center shadow-lg transition-all ${inWishlist ? 'bg-red-500 text-white' : 'bg-white text-gray-600 hover:bg-red-50 hover:text-red-500'} ${isHovered ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-4'} transition-all duration-300`}>
          ...
          <motion.button
            ...
            className={`absolute bottom-0 left-0 right-0 bg-gradient-to-r from-red-600 to-red-600 text-white py-3 text-sm font-semibold flex items-center justify-center gap-2 transition-all duration-300 ${isHovered ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'} disabled:opacity-50`}
          >
